class MathOperation{
    ...
    numberToStringInSlots = (num) => {
        if (typeof num !== 'string') { return 'the input should be in string type.'}
        let res = {};
        ...
        if (num1 instanceof Object && num2 instanceof Object){
            ...
            for(let key in sumObject){
                if (sumObject[key] >= 10) {
                    ...
                    if (!sumObject[nextKey]) {sumObject[nextKey] = 0};
    ...
    cross = (num1, num2) => {
        ...
        if (num1 instanceof Object && num2 instanceof Object){
            ...
            while (num1[i]){
                let j = 0;
                while (num2[j]){
                    if (!sumObject[i+j]) { sumObject[i+j] = 0};
    ...
    division = (num, dividedBy) => {
        ...
        if (num instanceof Object){
            ...
            let res = result;
            while (result[i] === '0'){
                if (result[i+1] !== '0') {res = result.slice(i+1)};
                i++
            }
            if (Number(residual) === 0 ) { residual = 0};
    ...
    toPower10Sqrt = (num) => {
        ...
        return isNumLengthEven ? this.power('10',+numLength/2+'') : this.power('10',(+numLength+1)/2+'')
    }
